...
微信小程序：专业健身训练记录工具。记录每次训练的动作、组数、重量、次数，自动计算训练容量（Volume）与个人纪录（PR），生成周容量趋势与部位训练分布统计。内置 150 个动作的专业动作库、14 篇健身知识文章与 5 套训练计划模板。
...
- 数据层与页面层分离：`utils/util.js` 纯函数计算容量/统计，node 可单测（`node test.js`，85 项断言）
