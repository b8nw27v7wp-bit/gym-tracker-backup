...
微信小程序：专业健身训练记录工具。记录每次训练的动作、组数、重量、次数，自动计算训练容量（Volume）与个人纪录（PR），生成周容量趋势、训练热力图与部位训练分布统计。内置 180 个动作的专业动作库、14 篇健身知识文章与 5 套训练计划模板。
...
### 训练计划模式
- 5 套计划（新手全身 A/B/C、PPL 推拉腿、上下肢 A/B、减脂 4 日、居家无器械 3 日）共 17 个训练日
...
- 部位训练分布、招牌动作 PR（最大重量 / 最佳单组 / 最新估算 1RM）
...
- 数据全部存于 wx.setStorageSync，单用户本地数据模型，带 schema 版本迁移机制（v1→v2）
- 数据层与页面层分离：`utils/util.js` 纯函数计算容量/统计，node 可单测（`node test.js`，109 项断言）
