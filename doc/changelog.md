...
- **fix(ui)**: 剪贴板接口补 fail 兜底——data 导出备份、export 复制内容两处 `setClipboardData` 新增 fail 回调（隐私未授权/未声明时明确提示，而非静默失败）；`getClipboardData`（导入）与相册保存已有兜底不动
...
- **feat(train)**: 练后总结浮层（正反馈瞬间）——保存后弹出本次总结：总容量/组数/动作数/时长 + 较上次训练容量对比（+X%/-X%/持平）+ 刷新动作纪录数（正式组最大重量超历史）+ 鼓励文案；点击遮罩/按钮关闭
- **feat(stats)**: 肌肉恢复卡新增「欠练：XX，去补上」直达——点击写入部位并切训练页自动筛选该部位（GROUP_TO_SITE 肌群→部位映射）
- **feat(knowledge)**: 知识详情页新增「学完就练」——按文章反查关联部位（MUSCLE_ARTICLES），展示该部位推荐动作，点击携带预选动作（pending_exercise）跳训练页一键记录，知识→行动闭环
- **test**: 主套件新增第 23 节（14 项）：内联加组复制上一组/同步草稿、删组/仅剩一组拦截、总结浮层字段（960kg/2组/1动作/1小时）、容量对比文案（+X%/delta-up）、新 PR 提示、统计欠练识别与直达、训练页消费欠练部位、文章关联动作与去记录跳转；729→743
- **docs**: README/architecture/testing/dev-guide 测试计数同步（合计 1587→1601）；changelog 记录
- 回归：test.js 743 项（3 连跑）+ 16 个专项脚本全绿

## v2.28（2026-08-16）— 使用逻辑修复：草稿自动保存/自重统计口径/登录弱化/时长如实

- **feat(train)**: 训练草稿自动保存与恢复（`gym_draft`）——添加/编辑/删除/排序/备注/计划填充/复制上次/编辑历史任一变更即持久化（剥离临时 uid），重进页面自动恢复并提示"已恢复上次未保存的草稿"；保存成功后清除；放弃编辑/清空数据同步丢弃
- **feat(util)**: 自重容量口径——显式自重（weight 为 0/空/未填）且有次数的组按次数计入容量（10 次=10 单位），自重训练者的容量/PR/周目标不再恒 0；脏数据（NaN/对象 weight）仍归零（安全回归保持）
- **feat(profile)**: 数据卡补"数据仅保存在本机，卸载/换机前请先导出备份"提示；训练日提醒文案如实改为"开 · 应用内提醒"（订阅推送需后端，不再显示虚假的"已订阅"）；版本号 v1.0.0 → v2.28
- **fix(login)**: 本地昵称不再 30 天过期——纯本地工具无后端，强制重登是多余摩擦（app.js 删除过期清除逻辑，isLoginValid 不再依赖 loginTime）
- **fix(train)**: 未计时/不足 1 分钟保存不再写假时长"1 分钟"——不写 duration 字段，历史显示"未计时"
- **fix(regression)**: 修复添加动作后不进入组编辑态的回归（enterEdit 调用丢失）
- **test**: 主套件新增第 22 节（12 项）：草稿持久化/剥离 uid/备注同步/删光清除/退出恢复+提示/不重复恢复/保存清除/未计时时长；自重容量断言更新（w2 40→52、本周 2612、背 52、自重 0×12=12）；717→729
- **docs**: architecture.md 存储 key 注册 `gym_draft`（合计 1575→1587）；testing/design 自重容量口径同步；changelog 记录
- 回归：test.js 729 项（3 连跑）+ 16 个专项脚本全绿
